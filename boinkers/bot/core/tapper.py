    ...
    async def get_tg_web_data(self, proxy: str | None) -> str:
        ...
        try:
            ...
            if settings.USE_REF == True:
                ...
                ref_id = 'boink2024951199'

            self.start_param = random.choices([ref_id, "boink2024951199"], weights=[90, 10], k=1)[0]
            ...
            try:
                ...
                if user_info is not None:
                    if user_info['boinkers'] and 'completedBoinkers' in user_info['boinkers']:
                        logger.info(f"<light-yellow>{self.session_name}</light-yellow> | ForestArmy Boinkers: <light-blue>{user_info['boinkers']['completedBoinkers']}</light-blue> 👨‍🚀")

                    if 'currencySoft' in user_info:
                        logger.info(f"<light-yellow>{self.session_name}</light-yellow> | ForestArmy Coin Balance: 💰 <light-green>{'{:,}'.format(user_info['currencySoft'])}</light-green> 💰")

                    if 'currencyCrypto' in user_info:
                        logger.info(f"<light-yellow>{self.session_name}</light-yellow> | ForestArmy Shit Balance: 💩 <cyan>{'{:,.3f}'.format(user_info['currencyCrypto'])}</cyan> 💩")
                    ...
                    if not last_claimed_time_x29 or current_time > last_claimed_time_x29 + timedelta(hours=2, minutes=5):
                        ...
                        if success:
                            logger.success(f"<light-yellow>{self.session_name}</light-yellow> | ForestArmy 🚀 Claimed boost successfully 🚀")
                ...
                logger.info(f"<light-yellow>{self.session_name}</light-yellow> | @forestarmy💤 sleep 30 minutes 💤")
                await asyncio.sleep(delay=1800)

            except Exception as error:
                logger.error(
                    f"<light-yellow>{self.session_name}</light-yellow> | @forestarmy😢 Unknown error: {error}")
